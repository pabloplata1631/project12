-- =============================================
--  Title: Insurance Verification Daily Action Data Source
--  Description: Daily data source for insurance verification actions
-- =============================================
/*
    1/5/2026 - Pablo Plata
    Notes - Has Primary Eligibility Checked Naming Convention 
YEAR(ad.primary_eligibility_checked_date) = YEAR(GETDATE())
		AND MONTH(ad.primary_eligibility_checked_date) = MONTH(GETDATE())
		AND ad.primary_insurance_group_name LIKE '[A-Z][A-Z]-[A-Z][A-Z][A-Z]-[0-9][0-9][0-9][0-9][0-9][0-9]-[A-Z0-9][0-9][0-9][0-9][0-9][0-9]'
*/
...
WHERE 1=1
	AND NOT (
		-- Exclude records where eligibility was checked this month AND naming convention matches
		YEAR(ad.primary_eligibility_checked_date) = YEAR(GETDATE())
		AND MONTH(ad.primary_eligibility_checked_date) = MONTH(GETDATE())
		AND ad.primary_insurance_group_name LIKE '[A-Z][A-Z]-[A-Z][A-Z][A-Z]-[0-9][0-9][0-9][0-9][0-9][0-9]-[A-Z0-9][0-9][0-9][0-9][0-9][0-9]'
	)