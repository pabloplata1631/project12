/*
    ...
    Notes 	UPPER CASE added to insurance fields to standardize case
...
FROM (
	SELECT
		...
		a.appointment_date,
                ...
                a.appointment_start as appointment_time_24h,
...
(
			SELECT 	
				ad.appointment_id_source AS appointment_id_source_AD ,
				...
				,Upper(ad.primary_insurance) as primary_insurance
				,Upper(ad.primary_insurance_group_name) as primary_insurance_group_name
				...
				,Upper(ad.secondary_insurance) as secondary_insurance
				,Upper(ad.secondary_insurance_group_name) as secondary_insurance_group_name
...
mart.appointment_detail ad
		...
		AND NOT (
						...
						OR ad.primary_insurance  LIKE'%EDP%'
						OR ad.primary_insurance  LIKE '%SBS CAREINGTON DISCOUNT%'
						OR ad.primary_insurance  LIKE '%SMILIST%'
						OR ad.primary_insurance  LIKE '%IDG%'
						OR ad.primary_insurance  LIKE '%EMPLOYEE FAMILY PLAN%'
						OR ad.primary_insurance  LIKE '%EMPLOYEE NAVIGATOR%'
						OR ad.primary_insurance  LIKE '%EMPLOYEE PLAN%'
						OR ad.primary_insurance  LIKE '%ENCORE EMPLOYEE%'
						OR ad.primary_insurance  LIKE '%SMILIST EMPLOYEE%'
						OR ad.primary_insurance  LIKE '%SMILIST EMPLOYEE FAMILY%'
		)
) AD
	ON a.appointment_id_source = ad.appointment_id_source_ad
